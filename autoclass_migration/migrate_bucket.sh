...
if gsutil -u $GCP_PROJECT autoclass get gs://$BUCKET | grep True; then
    post_to_slack "Autoclass is already enabled for $BUCKET"
    exit 0
fi

# Store the IAM permissions.
gsutil -u $GCP_PROJECT iam get gs://$BUCKET > /tmp/iam.json

# Store the object versioning setting.
OBJECT_VERSIONING=$(gsutil -u $GCP_PROJECT versioning get gs://$BUCKET | cut -f 2 -d ' ')

# Store the lifecycle configuration.
gsutil -u $GCP_PROJECT lifecycle get gs://$BUCKET > /tmp/lifecycle_config.json

# Store the Requester Pays setting.
REQUESTER_PAYS=$(gsutil -u $GCP_PROJECT requesterpays get gs://cpg-fewgenomes-test | cut -f 2 -d ' ')

# Remove all IAM permissions to prevent modifications while we perform the temporary copy.
gsutil -u $GCP_PROJECT iam set -e '' <(echo "{}") gs://$BUCKET

# Determine total size.
BUCKET_SIZE=$(gsutil -u $GCP_PROJECT du -s gs://$BUCKET | cut -f 1 -d ' ')
...
if [[ BUCKET_SIZE -gt 0 ]]; then
    ...
    gcloud --project=$GCP_PROJECT --billing-project=$GCP_PROJECT storage buckets create gs://$TMP_BUCKET \
        --location=australia-southeast1 \
        --uniform-bucket-level-access

    # Copy all data to the temporary bucket.
    gcloud --billing-project=$GCP_PROJECT storage cp -r "gs://$BUCKET/*" "gs://$TMP_BUCKET"

    # Compare total bucket sizes to make sure the copy completed successfully.
    TMP_BUCKET_SIZE=$(gsutil -u $GCP_PROJECT du -s gs://$TMP_BUCKET | cut -f 1 -d ' ')
...
gcloud --billing-project=$GCP_PROJECT storage rm --recursive gs://$BUCKET/

# Recreate the bucket, this time with Autoclass enabled.
gcloud --project=$GCP_PROJECT --billing-project=$GCP_PROJECT storage buckets create gs://$BUCKET \
    --location=australia-southeast1 \
    --uniform-bucket-level-access \
    --enable-autoclass
...
if [[ BUCKET_SIZE -gt 0 ]]; then
    ...
    gcloud --billing-project=$GCP_PROJECT storage cp -r "gs://$TMP_BUCKET/*" "gs://$BUCKET"
    ...
    gcloud --billing-project=$GCP_PROJECT storage rm --recursive gs://$TMP_BUCKET/
fi

# Restore object versioning.
if [[ OBJECT_VERSIONING = "Enabled" ]]; then
    gsutil -u $GCP_PROJECT versioning set on gs://$BUCKET
fi

# Restore the lifecycle configuration.
if ! grep "has no lifecycle configuration" /tmp/lifecycle_config.json; then
    gsutil -u $GCP_PROJECT lifecycle set /tmp/lifecycle_config.json gs://$BUCKET
fi

# Restore the Requester Pays setting.
if [[ REQUESTER_PAYS = "Enabled" ]]; then
    gsutil -u $GCP_PROJECT requesterpays set on gs://$BUCKET
fi

# Restore the IAM permissions.
gsutil -u $GCP_PROJECT iam set -e '' /tmp/iam.json gs://$BUCKET
