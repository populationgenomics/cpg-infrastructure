...
from cpg_infra.driver.dynamic_providers.seqera.inputs.compute_environment import (
    ComputeEnvArgs, GoogleBatchConfig,
)
...
def _create_compute_env(workspace_id: int, ce_body: dict) -> str:
    ...
    result = call_seqera_api(
        "POST", f"/compute-envs?workspaceId={workspace_id}", ce_body
    )
...
def _update_compute_env_metadata(
    workspace_id: int, ce_id: str, name: str
) -> None:
    """
...
def _soft_delete_compute_env(workspace_id: int, ce_id: str, current_name: str) -> None:
    ...
    assert len(deprecated_name) <= 100
...
class _ComputeEnvProvider(ResourceProvider):
    ...
    def update(
        ...
        return UpdateResult(outs={**news, "compute_env_id": id_})
