...
def _update_credentials(id_: str, inputs: GoogleCredentialsArgs) -> str | None:
    """
...
def _delete_credentials(id_: str, workspace_id: int) -> None:
    ...
    call_seqera_api(
        "DELETE",
        f"/credentials/{id_}?workspaceId={workspace_id}"
    )
...
class _GoogleCredentialProvider(ResourceProvider):
    def create(self, props: dict[str, Any]) -> CreateResult:
        ...
        if not cred_id:
            raise ValueError("Credential create did not return credentialsId")
    ...
    def delete(self, id_: str, props: dict[str, Any]) -> None:
        try:
            ...
            if e.status_code == 404:
                pulumi.log.info(f"Google credential {id_} already deleted/not found. Skipping delete.")
            else:
                raise
